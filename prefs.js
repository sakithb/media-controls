...
export default class MediaControlsPreferences extends ExtensionPreferences {
  _initWidgets(settings, builder) {
    let widgetPreset = builder.get_object("sepchars-preset");
    let widgetCustom = builder.get_object("sepchars-custom");
    let trackLabelStart = builder.get_object("track-label-start");
    let trackLabelSep = builder.get_object("track-label-sep");
    let trackLabelEnd = builder.get_object("track-label-end");
    let widgetCacheSize = builder.get_object("cache-size");
    let widgetElementOrder = builder.get_object("element-order");
    let widgetBacklistBox = builder.get_object("backlist-listbox");
  ...
  _bindSettings(settings, builder) {
    settings.bind(
      "max-widget-width",
  ...
  fillPreferencesWindow(window) {
    ...
    builder.add_from_file(this.path + "/prefs4.ui");

    this._initWidgets(window._settings, builder);
    this._bindSettings(window._settings, builder);
...
const MediaControlsBuilderScope = GObject.registerClass(
