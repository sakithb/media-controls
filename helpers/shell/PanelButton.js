...
/**
 * Adds a tap/click handler to an actor. Uses Clutter.TapAction when available,
 * otherwise falls back to a 'button-press-event' handler so the extension works
 * on environments where TapAction isn't provided.
...
class PanelButton extends PanelMenu.Button {
    ...
    addMenuPlayers() {
        ...
        if (this.menuPlayersTextBoxPin == null) {
            ...
            add_tap_or_click_handler(this.menuPlayersTextBoxPin, () => {
        ...
        for (let i = 0; i < players.length; i++) {
            ...
            if (players.length > 1) {
                ...
                if (isSamePlayer) {
                    ...
                    add_tap_or_click_handler(icon, this.updateProxy.bind(this, player));
                }
    ...
    addMenuControls(flags) {
        ...
        if (flags & WidgetFlags.MENU_CONTROLS_SHUFFLE) {
            this.addMenuControlIcon(
                // Show the icon that reflects the current shuffle state (ON when true, OFF when false)
    ...
    addMenuControlIcon(options, reactive, onClick) {
        ...
        add_tap_or_click_handler(icon, onClick);
     ...
     */
    initActions() {
        // Prefer Clutter actions when available, but fall back to event handlers if
        // the action classes are not present in this Clutter build (some GNOME
        // versions remove TapAction).
        if (typeof Clutter.SwipeAction === 'function') {
            try {
                ...
                /* ignore swipe support if it fails */
            }
        }
        ...
        if (typeof Clutter.TapAction === 'function') {
            try {
                ...
                // fall back to event-based handling below
            }
        }

        // Fallback: if TapAction isn't available or failed to initialize, use
        // a button-press-event handler on the actor.
        if (typeof Clutter.TapAction !== 'function') {
            // Use release event to avoid conflicting with PanelMenu's internal
            // press/release behavior which can toggle the menu on release.
            this.connect('button-release-event', (_, event) => {
                const button = event.get_button ? event.get_button() : Clutter.BUTTON_PRIMARY;
                if (this.doubleTapSourceId != null) {
                    GLib.source_remove(this.doubleTapSourceId);
                    this.doubleTapSourceId = null;
                    this.doMouseAction(this.extension.mouseActionDouble);
            ...
            // If TapAction exists, keep scroll-event handling as before
            this.connect('scroll-event', (_, event) => {
                const direction = event.get_scroll_direction();
                if (direction === Clutter.ScrollDirection.UP) {
                    this.doMouseAction(this.extension.mouseActionScrollUp);
