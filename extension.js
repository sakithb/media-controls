...
export default class MediaControls extends Extension {
    ...
    updateMediaNotificationVisiblity(shouldReset = false) {
        try {
            // Defensive access to fragile internal objects that may change between GNOME versions
            ...
            if (
                ...
                // @ts-expect-error - internal GNOME objects
                mediaSource = Main.panel.statusArea.dateMenu._messageList._messageView._mediaSource;
            }

            if (this.mediaSectionAddFunc && (shouldReset || this.hideMediaNotification === false)) {
                if (Mpris && Mpris.MprisSource && Mpris.MprisSource.prototype && Mpris.MprisSource.prototype._addPlayer) {
                    Mpris.MprisSource.prototype._addPlayer = this.mediaSectionAddFunc;
                }
                this.mediaSectionAddFunc = null;
                if (mediaSource && typeof mediaSource._onProxyReady === "function") {
                    mediaSource._onProxyReady();
                }
            } else {
                if (Mpris && Mpris.MprisSource && Mpris.MprisSource.prototype) {
                ...
                if (mediaSource && mediaSource._players != null && typeof mediaSource._onNameOwnerChanged === "function") {
            ...
            // Use extension's error logger to capture the problem but avoid throwing
            try {
                errorLog("Failed to update media notification visibility defensively:", e);
            } catch (_) {
                log("mediacontrols: defensive update failed: " + e);
            }
        }
    }
